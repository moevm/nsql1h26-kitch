...
import { useCreateWorker } from '../../../hooks/useWorkers.ts';
import type { WorkerCreate } from '../../../types/worker.ts';
...
const INITIAL_FORM_STATE: WorkerCreate = {
    name: '',
    email: '',
    date_of_birth: '',
    position: '',
...
export function AddEmployeeModal({ open, onClose }: AddEmployeeModalProps): ReactElement {
    const createWorker = useCreateWorker();
    const [formData, setFormData] = useState<WorkerCreate>(INITIAL_FORM_STATE);
    ...
    const clearFieldError = (field: keyof WorkerCreate) => {
        if (errors[field]) {
    ...
    const handleChange = (field: keyof WorkerCreate) => (value: string) => {
        const processedValue = field === 'start_experience'
    ...
    const handleStartTimeChange = (value: string) => {
        ...
        clearFieldError('work_day_start');
    };

    const handleEndTimeChange = (value: string) => {
        const formatted = formatTime(value);
        setFormData(prev => ({ ...prev, work_day_end: formatted }));
        clearFieldError('work_day_end');
    };
        ...
        <Modal open={open} onClose={handleClose}>
            <Box sx={modalStyle}>
                ...
                <CommonInputField
                    label="Дата рождения *"
                    type="text"
                    placeholder="ГГГГ-ММ-ДД (только цифры)"
                ...
                <CommonInputField
                    ...
                    type="text"
                    placeholder="0"
                    value={String(formData.start_experience)}
                    onChange={(v) => handleChange('start_experience')(v)}
                    error={!!errors.start_experience}
                    helperText={errors.start_experience}
                />

                <CommonInputField
                    label="Начало смены *"
                    placeholder="ЧЧММ (только цифры)"
                ...
                <div style={{ display: 'flex', gap: 16, marginTop: 24 }}>
                    <CommonButton
                        title={createWorker.isPending ? 'Добавление...' : 'Добавить'}
